...
std::pair<double, std::vector<const Node*>> find_path_between_tarns(
  ...
  // std::cout << tarn1.name << ": (" << start->get_location().first << "," <<
  // start->get_location().second << ") " << tarn2.name << ": (" <<
  // goal->get_location().first << "," << goal->get_location().second << ")" <<
  // std::endl;
...
double tsp(const int mask, const int pos, const int n,
  ...
  for (int i = 0; i < n; i++) {
    if ((mask & (1 << i)) == 0) {
      ...
      ans = std::min(ans, dist[pos * n + i] + tsp(mask | (1 << i), i, n,
                                                  min_dist_per_day, dist, dp));
    }
  }
...
reconstruct_path(const std::vector<TarnData>& tarns,
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    path.push_back(
        std::make_pair(tarns[pos], paths.at(n * pos + next_pos).size()));
    // std::cout << "Adding path from " << tarns[pos].name << " to " <<
    // tarns[next_pos].name << " to path" << std::endl;
    if (next_pos < pos) {
      // std::cout << "Reversing path" << std::endl;
      std::reverse(paths[n * pos + next_pos].begin(),
                   paths[n * pos + next_pos].end());
    }
    path_nodes.insert(path_nodes.end(), paths.at(n * pos + next_pos).begin(),
                      paths.at(n * pos + next_pos).end());

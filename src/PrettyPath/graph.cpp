...
void Graph::add_edge(const Node* node1, const Node* node2, const double length, const double slope, const int cars, const long osm_id, const std::vector<node_id_t> edge_nodes) {
...
std::vector<std::pair<const Node*, const Edge>> Graph::get_neighbours(const Node* node) const {
    return m_graph.at(node);
}

std::pair<const Node*, double> Graph::find_closest_node(const double latitude, const double longitude) const {

...
void download_map_tile(const std::string& url, const std::string& output_filename) {
    ...
    if (res != CURLE_OK) {
        std::cerr << "Error: Failed to download " << url << ": " << curl_easy_strerror(res) << std::endl;
    }
...
int longitude_to_pixel(double longitude, double min_lon, double max_lon, int image_width) {
    return (longitude - min_lon) / (max_lon - min_lon) * image_width;
}

int latitude_to_pixel(double latitude, double min_lat, double max_lat, int image_height) {
    return (max_lat - latitude) / (max_lat - min_lat) * image_height;
}

void plot_path_on_map(const std::string& image_filename, const std::vector<Node*>& path, double min_lat, double max_lat, double min_lon, double max_lon) {
    // Load the image

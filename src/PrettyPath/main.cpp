...
int main(int argc, char** argv) {
    ...
    auto filtered_tarns = TarnRouter::filter_tarns(tarns, Config::c.minimum_tarn_elevation, Config::c.maximum_tarn_elevation, Config::c.minimum_tarn_area, Config::c.minimum_latitude, Config::c.maximum_latitude, Config::c.minimum_longitude, Config::c.maximum_latitude);
    std::cout << "Tarns must have an elevation between " << Config::c.minimum_tarn_elevation << " and " << Config::c.maximum_tarn_elevation<< " m" << std::endl;
    std::cout << "Tarns must have an area of at least " << Config::c.minimum_tarn_area << " m^2" << std::endl;
    ...
    std::cout << std::endl;
    auto path = TarnRouter::find_shortest_path_between_tarns(graph, filtered_tarns, Config::c.min_dist_per_day);
